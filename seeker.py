...
parser.add_argument('-p', '--port', type=int, default=1234, help='Web server port [ Default : 1234 ]')
...
def ngrok_server():
	try:
		print(f"{G}[+] {C}Starting ngrok")
		public_url = ngrok.connect(1234, "http").public_url
		print(f"{G}[+] {C}Public url: {public_url}")
		print(f"{G}[+] {C}Local url: http://localhost:1234\n")
	except Exception as e:
		print(f"{Y}[!] Ngrok error:")
		print("  "+str(e).replace("\n", "\n  ")+"\n")
